...
function resolveAnchorDomainForExecution(domain) {
...
function resolveClientDomain(req) {
    const explicit = process.env.SEP10_CLIENT_DOMAIN?.trim();
    if (explicit)
        return toHostname(explicit);
    const webOrigin = process.env.WEB_ORIGIN?.trim();
    if (webOrigin)
        return toHostname(webOrigin);
    const forwardedHost = req.headers["x-forwarded-host"]?.trim() ?? "";
    if (forwardedHost)
        return toHostname(forwardedHost);
...
function getCallbackSecret() {
    ...
    if (getPopEnv() === "production" &&
        (secret.length < 24 || secret.toLowerCase().includes("change_me"))) {
        throw new Error("ANCHOR_CALLBACK_SECRET is required and must be strong in production.");
    }
    return secret;
}
...
function buildCallbackUrl(req, transactionId, callbackToken) {
...
function getExecutionStateSecret() {
    const secret = process.env.EXECUTION_STATE_SECRET?.trim() ?? "";
    if (!secret) {
        throw new Error("Missing EXECUTION_STATE_SECRET in backend env. Required to protect SEP-24 status polling state.");
    }
    if (getPopEnv() === "production" &&
        (secret.length < 24 || secret.toLowerCase().includes("change_me"))) {
        throw new Error("EXECUTION_STATE_SECRET is too weak for production. Use a strong random secret.");
    }
    return secret;
}
...
function encryptStatusRef(payload) {
...
function decryptStatusRef(statusRef) {
    const secret = getExecutionStateSecret();
    const key = createHash("sha256").update(secret).digest();
...
async function exchangeSep10Token(input) {
    ...
    if (!response.ok) {
        const raw = await response.text();
        throw new Error(`SEP-10 token exchange failed at ${webAuthEndpoint} (${response.status}): ${raw || response.statusText}`);
    }
    const payload = (await response.json());
    if (!payload.token) {
...
async function fetchSep24TransactionStatus(handle) {
    ...
    const endpoint = `${transferServer}/transaction?id=${encodeURIComponent(handle.interactiveId)}`;
    const response = await fetch(endpoint, {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
        const raw = await response.text();
        throw new Error(`SEP-24 transaction status failed at ${transferServer} (${response.status}): ${raw || response.statusText}`);
    }
    const payload = (await response.json());
    const tx = payload && typeof payload.transaction === "object" && payload.transaction
        ? payload.transaction
        : payload;
    const status = typeof tx.status === "string"
        ? tx.status
        : typeof tx.state === "string"
            ? tx.state
            : undefined;
    const stellarTxHash = typeof tx.stellar_transaction_id === "string"
        ...
        : typeof tx.stellarTransactionId === "string"
            ...
            : typeof tx.stellar_transaction_hash === "string"
                ...
                : undefined;
    const externalTransactionId = typeof tx.external_transaction_id === "string"
...
export default async function handler(req, res) {
    ...
    const phase = asString(parsed.value.phase);
    ...
    try {
        ...
        if (phase === "status") {
            ...
            const results = await Promise.all(state.anchors.map(async (handle) => {
                try {
        ...
        const signatures = parsed.value.signatures;

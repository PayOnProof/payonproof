...
export async function resolveAnchorCapabilities(input: {
  ...
  if (sep1.transferServerSep24) {
    try {
      ...
      diagnostics.push(
        `SEP-24 /info error: ${error instanceof Error ? error.message : "unknown"}`
      );
    }
  } else {
  ...
  if (sep1.transferServerSep6) {
    try {
      ...
      diagnostics.push(
        `SEP-6 /info error: ${error instanceof Error ? error.message : "unknown"}`
      );
    }
  } else {
  ...
  if (!sep.sep31) diagnostics.push("SEP-31 endpoint missing in stellar.toml");
